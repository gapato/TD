...
suivants sont des espaces vectoriels. Dans le cas o\`u $E$ est de
dimension finie, en donner la dimension, et un procédé de construction
de base à partir de celles de $S_1$ et $S_2$.
\begin{align*}
  E_1 &= S_1 \cap S_2\\
  E_2 &= S_1 \cup S_2\\
...
Réécrire ce problème sous la forme d'une équation matricielle $V A =
Y$, o\`u $A$ est le vecteur des coefficients du polynome $P$ : $P(X) =
a_0 + a_1 X + \dots + a_{n-1} X^{n-1}$. La matrice $V$ est appelée
matrice de Vandermonde, et le calcul de son déterminant est un
exercice classique en théorie des
déterminants\footnote{\url{http://fr.wikipedia.org/wiki/Matrice_de_Vandermonde}}. Démontrer
que si les $x_i$ sont tous distincts, la matrice $V$ est inversible,
et ainsi prouver l'existence et l'unicité du problème d'interpolation
polynomiale. Indice : on pourra montrer que si $V A = 0$, alors $P$
est nul. Que se passe-t-il si on considère des polynomes de degré
supérieur ou inférieur à $n-1$ ?

\exercice (Polynomes trigonométriques) Montrer que $P$, l'espace des
fonctions continues périodiques de période $2\pi$, est un espace
vectoriel. Quelle est sa dimension ? Montrer que la famille $(\sin(x),
\sin(2x), \sin(3x))$ est une sous- famille libre de $P$. Est-elle
génératrice ? On peut montrer qu'en un certain sens, la famille
...
fonctions continues de période $2\pi$, muni du produit scalaire (le
démontrer)
\begin{align*}
  <f,g> &= \int_0^{2\pi} f g.
\end{align*}
...
$F$:
\begin{align*}
  \frac{d}{d \varepsilon} F(x+\varepsilon y) = 0  
\end{align*}
En déduire que, pour le minimum, on a
\begin{align*}
  A^T A x = A^T b
\end{align*}
