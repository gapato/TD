...
\exercice (Norme induite) Soit $f$ une application linéaire sur $(E,\norm{.})$ de dimension
finie. On définit peut définir la \emph{norme induite} de $f$ par :
\begin{align*}
	N(f) = \sup \left\{\frac{\norm{f(x)}}{\norm{x}}\; \Big|\; x\in E\right\} =  \sup
	\left\{\norm{f(x)} \;\big|\; x\in
	E,\;\norm{x} = 1\right\}.
\end{align*}
...
\exercice (Normes de matrices) En considérant la représentation matricielle d'une application
linéaire, on peut définir une norme sur $\mathcal{M}_n(\mathbb{R})$ pour chaque norme sur
$\mathbb{R}^n$.
\begin{itemize}
