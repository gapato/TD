...
les signaux composites enregistr\'{e}s dans \tt R par des combinaisons
affines de ces m\^{e}mes signaux, de telle sorte que ces signaux
soient centrés et de matrice de dispersion égale à la matrice
...
\item Entrer la commande : \tt{cppfun([1 0 0]')}. Quelle quantité
  cette commande calcule-t-elle ?  Entrer la commande : \tt{cppfun(eye(3))}. Elle retourne les valeurs respectives
...
\item \textbf{Choix du coefficient $\mu$}\\
  ...
  la majoration issue de l'inégalité de Cauchy-Schwartz
\[
M\leq C=\frac{1}{N} \sum_{i=1}^N \left( \sum_{j=1}^3 (X_i^j)^2\right)^2.
\]

\onit
\item Pourquoi cette majoration est-elle extrêmement grossière ?
  Quelle influence le choix d'une majoration trop élevée a-t-il sur
  l'algorithme ?
...
ordre de grandeur \og raisonnable \fg\ pour le pas.
