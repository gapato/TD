...
oublie les signaux originaux \tt R.
\offit
...
\item Quels compromis faut-il faire dans le choix des différents
  paramètres ?
\item Conclure sur la solution du problème obtenue avec cette
  méthode. Quels sont les avantages et inconvénients de la méthode ?
  \offit \offit
...
\item  Finalement, le dernier signal sera obtenu par un nouvel appel \`a la fonction \tt{prune}, et ajout\'{e} dans \tt S par la commande : \tt{prune(w); S(:,3)=X*w;}. Vous disposez maintenant d'une approximation \tt S de la matrice des signaux sources, et vous pouvez :
\onit
