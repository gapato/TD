...
\[
F(w)=E\left[(R\star w)^4\right]=\frac{1}{N}\,\sum_{i=1}^N \left( \sum_{j=1}^3 R_i^j w_jÊ\right)^4
\]
...
\onit
\item T\'{e}l\'{e}charger le fichier-archive \tt{optnum-tp1.tgz} \`{a} l'adresse suivante :

\centerline{\url{http://docs.ufrmd.dauphine.fr/optinum/tps/optinum-tp1.tgz}}

puis utiliser la commande \textsc{unix} \tt{tar -xvfz optnum-tp1.tgz} pour cr\'{e}er le r\'{e}pertoire
\tt{optinum-tp1} et en extraire les fichiers qui seront utilis\'{e}s durant la s\'{e}ance. Lancer alors
...
\[
E\left[(R\star w)^4\right]- \mu\,\left(\|w\|^2-1\right)^2 =\frac{1}{N}\,\sum_{i=1}^N \left( \sum_{j=1}^3 R_i^j w_jÊ\right)^4
