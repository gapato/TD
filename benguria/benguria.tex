...
\frame{
  ...
  \begin{itemize}
  \item Sous-problème : minimiser
    \begin{equation*}
      F(\psi) = \frac{a^2 \int (\nabla \psi)^2 \dd x + b^2
        \int \psi^{4} \dd x}{
        \alpha z \int \frac{\psi^3(x)}{|x|} \dd x}
    \end{equation*}
  \item Équivalent à $\mathcal E$ en négligeant l'interaction
    électronique $D(\rho,\rho)$
  \item Raisonable parce que faible physiquement, et pas indispensable
    mathématiquement
  \item Invariant par scaling
  \item $\inf F$ résoluble explicitement par réduction au cas radial.
  \item $\inf F = \frac{4 a b}{3 \alpha z}$
...
\frame{
  ...
  \begin{itemize}
  \item Regarder $F$ sous $\psi \rightarrow \psi^*$, où $\psi^*$ est
    le réarrangement symétrique (ref: Lieb \& Loss, Analysis)
  ...
  \item Propriétés
    ...
    \end{enumerate}
  \item $F(\psi^*) \le F(\psi)$
  \item Sans perte de généralité, $\psi$ radiale décroissante
  \end{itemize}
}
...
\frame{
  ...
  \item $F(\psi) \ge \frac{4 a b}{3 \alpha z}$
  \item On remonte le cas d'égalité $x = - y$, on en déduit l'équation
    différentielle $a \psi' = - b \psi^2$ qu'on résoud:
    \begin{equation*}
      \psi(r) = \phi_{R}(r) = \frac a b \frac {1}{r+R}
    \end{equation*}
  \item $\inf F(\psi) = F(\phi_R) = \frac{4 a b}{3 \alpha z}$,
    $\phi_R$ unique minimiseur à scaling près
  \end{itemize}
}
...
\frame{
  ...
  \item Si $F_0 < 1$, alors l'argument ne permet pas de conclure à
    l'instabilité
  \item Mais il est naturel de tester $\mathcal E$ sur $\phi_R$, qui
...
\frame{
  ...
  \item On a donc $\frac{4 a b}{3 \alpha} \le z_c \le \frac{4 a b}{3 \alpha} + \frac{7 \pi
    ...
    \item L'encadrement est bon. L'interaction entre les électrons est
      faible, et l'approximation $D(\rho,\rho) = 0$ était physiquement raisonable
  \end{itemize}
}
